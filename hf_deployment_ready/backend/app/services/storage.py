    ...
    def __init__(self):
        ...
        if self.use_mongodb:
            if not settings.MONGODB_URL:
                logger.error("❌ MONGODB_URL is not set. Cannot connect to MongoDB. Falling back to JSON.")
                self.use_mongodb = False
            else:
                try:
                    from pymongo import MongoClient
                    # Thêm timeout ngắn để không bị treo nếu URL sai
                    ...
                    logger.error(f"❌ MongoDB connection failed: {e}. Falling back to JSON file.")
                    self.use_mongodb = False
